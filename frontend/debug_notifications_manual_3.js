...
async function run() {
    console.log("--- DEBUGGING NOTIFICATIONS (MATERIAL ONLY) ---");

    // Test Query with ONLY material
    const { data: qData, error: qError } = await supabase
        .from('notifications')
        .select('*, material:materials(name, part_number)')
    ...
    if (qError) {
        ...
        if (qData.length > 0) console.log("First item:", JSON.stringify(qData[0], null, 2));
    }
}
