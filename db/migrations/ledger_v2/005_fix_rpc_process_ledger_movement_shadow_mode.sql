-- db/migrations/ledger_v2/005_fix_rpc_process_ledger_movement_shadow_mode.sql
-- Fixes the JSONB boolean parsing in the shadow_mode bypass
...
BEGIN
    ...
    -- BUGFIX: In PSQL JSONB, a boolean true extracted with ->> becomes the string 'true', 
    -- BUT if it's extracted with -> it remains a jsonb boolean. We cast to text to be safe.
    -- Better yet, we just check if it literally equals 'true' returning from ->> text extraction.
    -- Furthermore, to avoid any possibility of the outer deliver_ticket failing from this RPC, we wrap the whole thing.
