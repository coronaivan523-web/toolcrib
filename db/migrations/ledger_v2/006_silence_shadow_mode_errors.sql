-- db/migrations/ledger_v2/006_silence_shadow_mode_errors.sql
-- Wraps the dual write injection in an exception block so that any
-- ledger calculation errors do not abort the legacy ticket delivery.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM public.ticket_items WHERE ticket_id = v_real_ticket_id LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
        -- SILENT FALLBACK: If Ledger V2 fails (e.g. baseline not loaded and validation breaks),
        -- do NOT rollback the entire legacy ticket delivery. Just log a warning conceptually.
        -- Plpgsql does not have a direct 'log warning', so we just swallow it in Shadow Mode.
        NULL;
    END;
